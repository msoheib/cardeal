...
const carModels: Record<string, string[]> = {
  ...
  'لكزس': ['إي إس', 'إل إس', 'آي إس', 'آر إكس', 'إل إكس', 'جي إكس', 'إن إكس'],
  'نيسان': ['باترول', 'صني', 'التيما', 'ماكسيما', 'اكس تريل', 'باثفندر'],
  'هوندا': ['أكورد', 'سيفيك', 'باي-لوت', 'سي آر-في'],
  'هيونداي': ['سوناتا', 'إلنترا', 'أكسنت', 'توسان', 'سنتافي', 'باليسيد'],
  'كيا': ['كي 5', 'سيراتو', 'ريو', 'سبورتاج', 'سورينتو', 'تيلورايد'],
  // Default fallback
  'other': ['موديل 1', 'موديل 2']
}
const years = Array.from({ length: 11 }, (_, i) => 2025 - i)
const trims = ['قياسي', 'جي إل', 'جي إل إكس', 'إكس إل إي', 'محدود', 'رياضي', 'بلاتينيوم', 'تيتانيوم', 'كامل المواصفات']
...
export default function AddCarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, confirmed: boolean = false) => {
    ...
    if (!formData.make || !formData.model || !formData.year || !formData.trim || !formData.color || !formData.origin_locale || !formData.wakala_price || !formData.quantity) {
      setError('يرجى ملء جميع الحقول المطلوبة (الشركة، الموديل، السنة، الفئة، اللون، المنشأ، السعر، الكمية)')
      setIsLoading(false)
      return
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                   <Label>منشأ السيارة *</Label>
                    <Select value={formData.origin_locale} onValueChange={(v) => handleInputChange('origin_locale', v)}>
                ...
                <div className="flex flex-wrap gap-2">
                    {priceSlots.map(p => (
                        <div key={p} className="flex items-center gap-2 bg-secondary px-3 py-1 rounded-full text-sm">
                            {formatNumber(p)} <X className="w-3 h-3 cursor-pointer" onClick={() => removePriceSlot(p)} />
                        </div>
                    ))}
                </div>
              </div>
