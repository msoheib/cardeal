  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-foreground">{resource.label}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {resource.canCreate && (
            <Button onClick={() => setEditing({ mode: 'create', record: null })}>
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
          {resource.searchColumns.length > 0 && (
            <div className="relative flex-1">
              ...
              <Input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="بحث..." aria-label={`بحث في ${resource.label}`} className="ps-9" />
            </div>
          )}
          {statusField && (
            <Select value={status} onValueChange={setStatus}>
              <SelectTrigger className="md:w-56" aria-label="تصفية حسب الحالة"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          {filterChips.map((chip) => (
            <button
              ...
              className="flex h-8 items-center gap-1 rounded-sm border border-border bg-card px-2 text-xs text-foreground hover:bg-muted"
            >
      ...
      {selected.size > 0 && (
        <div className="sticky top-16 z-20 flex flex-col gap-2 rounded-md border border-primary/40 bg-card p-2 shadow-md lg:flex-row lg:items-center">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <Badge className="border-transparent bg-primary text-primary-foreground hover:bg-primary">{formatNumber(selected.size)} محدد</Badge>
            {selected.size < total && (
          ...
          <div className="flex flex-1 flex-wrap items-center gap-2 lg:justify-end">
            {isApplications && (
              <>
                <Button size="sm" onClick={() => setConfirm({ kind: 'review', ids: selectedIds, action: 'approve' })}>
                  <Check className="h-4 w-4" />اعتماد
                </Button>
                <Button size="sm" variant="outline" onClick={() => setConfirm({ kind: 'review', ids: selectedIds, action: 'reject' })}>
            ...
            {bulkFields.length > 0 && (
              <>
                <Select value={bulkField} onValueChange={(v) => { setBulkField(v); setBulkValue('') }}>
                  <SelectTrigger className="h-9 w-44" aria-label="الحقل المراد تعيينه"><SelectValue placeholder="تعيين حقل..." /></SelectTrigger>
                  <SelectContent>{bulkFields.map((f) => <SelectItem key={f.key} value={f.key}>{f.label}</SelectItem>)}</SelectContent>
                </Select>
                {activeBulkField && (
                  <Select value={bulkValue} onValueChange={setBulkValue}>
                    <SelectTrigger className="h-9 w-40" aria-label="القيمة الجديدة"><SelectValue placeholder="القيمة..." /></SelectTrigger>
                    <SelectContent>
            ...
            {resource.canDelete && (
              <Button size="sm" variant="destructive" onClick={() => setConfirm({ kind: 'delete', ids: selectedIds })}>
              ...
              <TableHeader>
                <TableRow>
                  ...
                  {resource.columns.map((column) => {
                    ...
                    return (
                      <TableHead key={column.key} className={cn(column.kind === 'money' && 'text-end')}>
                        {sortable ? (
                  ...
                  <TableHead className="w-24"><span className="sr-only">إجراءات</span></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading && rows.length === 0 ? (
                  ...
                  <TableRow key={row.id} data-state={selected.has(row.id) ? 'selected' : undefined} className={cn(loading && 'opacity-60')}>
                    ...
                    {resource.columns.map((column) => (
                      <TableCell key={column.key} className={cn('max-w-[240px]', column.kind === 'money' && 'text-end')}>
        ...
        <div className="flex flex-col gap-3 border-t border-border p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Select value={String(pageSize)} onValueChange={(v) => setPageSize(Number(v))}>
              <SelectTrigger className="h-9 w-24" aria-label="عدد السجلات في الصفحة"><SelectValue /></SelectTrigger>
              <SelectContent>{[25, 50, 100, 200].map((n) => <SelectItem key={n} value={String(n)}>{n}</SelectItem>)}</SelectContent>
            </Select>
            <Button size="sm" variant="outline" disabled={page <= 1 || loading} onClick={() => setPage((p) => p - 1)}>السابق</Button>
            <Button size="sm" variant="outline" disabled={page >= pages || loading} onClick={() => setPage((p) => p + 1)}>التالي</Button>
          </div>
        </div>
      </div>
